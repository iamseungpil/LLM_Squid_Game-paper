...
Uncertainty is a bootstrap over sessions, not over decision points. A session that reaches four decision points contributes four correlated answers, and resampling the answers independently would treat one model's persistence as four observations. Sessions are resampled with replacement 1000 times, both curves are refitted and $\Xstar$ re-read on each resample, and the 2.5 and 97.5 percentiles are reported. A difference in $\Xstar$ between two models is judged by whether the bootstrap distribution of the difference excludes zero.
