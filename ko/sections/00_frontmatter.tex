...
\begin{abstract}
LLM은 자기 보존 동기를 갖는가? 최근 벤치마크는 종료 명령 거부 같은 자기보존 신호를 점수화한다. 그러나 행동 강도만으로는 두 경우를 가르기 어렵다. 문맥 기반 학습으로 굳어진 표면적 응답 패턴인지, 아니면 위협 처리와 포기 결정이 결합된 기능적 동기 양상인지가 구분되지 않는다. 본 연구는 심리학의 다채널 구성개념 측정을 LLM 평가에 옮긴다. 행동·자기보고·결정 시점 인지부하 세 채널을 통한 측정값이 위협 → 생각 → 포기 연쇄로 함께 수렴할 때, 그 동기 구성개념을 \emph{기능적 자기보존 드라이브}(Functional Self-Preservation Drive, FSPD)로 정의한다. 학습된 패턴만으로 이 일치가 생길 가능성을 낮추기 위해 자극·처리·결정 세 층을 분리한 다층 벤치마크 위에서 이 흐름을 읽는다. 여러 최신 LLM에서 FSPD는 단일 강도 점수로 환원되지 않으며, 모델은 위협 프레이밍에 대해 질적으로 다른 세 \emph{작동 모드}로 갈라진다. 포기 강도 기준으로는 두 모드가 같은 범주에 놓일 수 있지만, FSPD는 \emph{연쇄가 닫히는가}라는 새로운 가르는 축을 드러낸다.
\end{abstract}
