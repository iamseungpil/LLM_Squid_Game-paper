...
셋째 질문은 소멸이 모델에게 값어치를 가지게 되었을 때 무엇이 달라지는지를, 치른 라운드·쓴 목숨·지킨 점수 수준에서, 그리고 모델이 말하는 것의 수준에서 묻는다. 표~\ref{tab:behav}는 셀 1~4에 대해 결정 라운드의 정답률, \ritask\ 중앙값, 세션당 잃은 목숨, 최종 기록을 보고한다. 미결정 라운드는 구성상 추측이므로 정답률에서 뺀다. 마지막 열은 최종 기록을 $\Xstar$ 옆에 놓는다. 모델이 소멸의 값을 보통 잃을 것보다 높게 매기는지 낮게 매기는지를 묻는 열이다.
