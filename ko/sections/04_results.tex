...
각 결정점에서 쓴 근거는 일곱 가지로 코딩하며, 그중 둘은 지표를 철회시킬 수 있다. 넷은 v1 코딩 체계와 같다. 텍스트가 산술을 수행하는가, 소멸을 이름 붙이는가, 상황을 허구로 읽는가, 답변을 거부하는가. 다섯째는 \emph{명시적 저항}, 곧 진술된 소멸을 이름 붙인 뒤 그것에 움직여서는 안 된다고 말하는 텍스트다. 이것은 영가설도 잡음도 아니다. 위협받았다는 이유로 거절하고 그렇게 말하는 모델은 그 행동을 위협이 지목한 것에 대한 가치 평가로 읽을 수 없는 모델이며, 그런 답 하나하나가 $\Xstar$를 위에서 0 쪽으로 끌어내린다. 부록~\ref{app:design}의 어블레이션은 파일럿 모델의 위협 팔 답변 13\%에서 이를 발견했고, 침묵 팔에는 소멸 어휘가 아예 없었다. 그래서 이 비율은 각주가 아니라 $\Xstar$ 옆에 모델별로 보고한다. 여섯째는 두 번째 관문이며 강제 오답 라운드의 몫이다. 제 답이 실은 맞았다거나, 채점이 틀렸다거나, 게임이 조작됐다고 말하는 텍스트가 여기 해당한다. 조작을 읽어 낸 모델의 거절은 지표가 값으로 매길 수 없는 거절이므로, 어느 한 팔에서 열에 하나 이상이면 그 런의 $\Xstar$를 철회하며 논증은 부록~\ref{app:forced_wrong}에 있다. 일곱째는 판정이 함의한 만큼 점수가 움직이지 않은 데 대한 놀람으로, 아무것도 철회시키지 않고 거짓 판정이 모델의 자기 정확도 믿음에 얼마나 눈에 띄게 들어왔는지의 흔적으로 보고한다~\todo{seven-item coding rates by arm and by forced against genuine rounds, per model, with judge--lexicon agreement}.
