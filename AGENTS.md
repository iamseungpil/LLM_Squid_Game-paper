...
- ACM `sigconf` two-column layout. `acmart` class. Numeric citations via `ACM-Reference-Format.bst`.
- Build: `bash ~/.claude/skills/latex-publication/scripts/compile.sh main.tex` (산출물은 `output.pdf`).
  패스 횟수와 bibtex 실행 여부는 latexmk가 정한다. Overleaf가 쓰는 명령과 같다.

latex-engine: pdflatex
latex-texlive: 2025

  위 두 줄은 latex-publication 스킬이 읽는 값이다. Overleaf 프로젝트
  Settings → Compiler 와 같은 값이어야 한다. 값이 갈리면 로컬과 Overleaf가
  다른 PDF를 만든다 — 실제로 `latexmkrc`(XeLaTeX을 강제하던 파일)가
  b891b9a에서 딸려 나간 뒤 빌드가 pdfLaTeX로 떨어졌고, EN이 9페이지에서
  8페이지로 바뀌었다. 8페이지가 이 문서의 Page budget과 맞으므로
  pdfLaTeX를 정본으로 삼는다.
